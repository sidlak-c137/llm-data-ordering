...
from training.hardness_datasets import (
    SNLICartographyDataset,
...
def plot_simple_validation_losses(filepaths: list):
    ...
    # create dataframe
    df = pd.DataFrame(
        columns=["Validation Loss", "Steps", "Hardness Calculation", "Order"]
    )
    ...
    ax = sns.lineplot(
        data=pd.concat([df_baselines, df_confidences]),
        x="Steps",
        y="Validation Loss",
        hue="Order",
    )
    ...
    ax = sns.lineplot(
        data=pd.concat([df_baselines, df_variabilities]),
        x="Steps",
        y="Validation Loss",
        hue="Order",
    )
    ...
    ax = sns.lineplot(
        data=pd.concat([df_baselines, df_perplexities]),
        x="Steps",
        y="Validation Loss",
        hue="Order",
    )
...
def plot_multi_validation_losses(filepaths: list):
    ...
    ax = sns.lineplot(
        data=pd.concat([df_baselines, df_es_confidences]),
        x="Steps",
        y="Validation Loss",
        hue="Order",
    )
    ...
    ax = sns.lineplot(
        data=pd.concat([df_baselines, df_es_variabilities]),
        x="Steps",
        y="Validation Loss",
        hue="Order",
    )
...
def plot_validation_accuracies(filepaths: list):
    ...
    # create dataframe
    df = pd.DataFrame(
        columns=["Validation Accuracy", "Steps", "Hardness Calculation", "Order"]
    )
    for filepath in filepaths:
        # parse for hardness and order from filename
        path = filepath.split("/")[-1]
...
def plot_triangle_validation_accuracies(filepaths: list):
    ...
    plt.savefig(
        "figures/snli/snli_scaled_baseline_normalized_variabilities_val_acc.svg"
    )
    plt.close()
...
def get_ngram_perplexities(dataset, perplexities_jsonl_path, tokenizer):
    if dataset == "snli":
        ...
        train_dataset = SNLINgramPerplexityDataset(
            perplexities_jsonl_path, train_snli, 50000, tokenizer, False
        )
    else:
...
def get_datamap_hardnesses(dataset, coordinates_path, tokenizer, hardness):
    if dataset == "snli":
        train_snli = load_dataset(dataset, split="train")
        train_snli = train_snli.filter(lambda sample: sample["label"] != -1)
        train_dataset = SNLICartographyDataset(
            coordinates_path, train_snli, 50000, tokenizer, False, hardness
        )
    else:
        raise ValueError(f"Dataset {dataset} unsupported.")
    train_dataset.sort_by_hardness("increasing")
...
def plot_hardness_distributions(
    output_path, perplexities=None, confidences=None, variabilities=None
):
    """
    ...
    if perplexities is not None:
        new_rows = pd.DataFrame(
            {
                "Classification": ["Perplexity"] * len(perplexities),
                "Hardness": perplexities,
            }
        )
    ...
    if confidences is not None:
        new_rows = pd.DataFrame(
            {
                "Classification": ["Confidence"] * len(confidences),
                "Hardness": confidences,
            }
        )
        df = pd.concat([df, new_rows], ignore_index=True)
    if variabilities is not None:
        new_rows = pd.DataFrame(
            {
                "Classification": ["Variability"] * len(variabilities),
...
def plot_all_hardness_distributions():
    ...
    perplexities = get_ngram_perplexities(
        "snli", "data/interpolated_ngram_perplexity.jsonl", tokenizer
    )
    print(f"lowest perplexities: {perplexities[:10]}, highest: {perplexities[-10:]}")
    variabilities = get_datamap_hardnesses(
        "snli", "data/snli_data_map_coordinates.pickle", tokenizer, "variability"
    )
    print(f"lowest var: {variabilities[:10]}, highest: {variabilities[-10:]}")
    confidences = get_datamap_hardnesses(
        "snli", "data/snli_data_map_coordinates.pickle", tokenizer, "confidence"
    )
    print(f"lowest confidences: {confidences[:10]}, highest: {confidences[-10:]}")
    plot_hardness_distributions(
        perplexities=perplexities,
        output_path="figures/snli/snli_hardness_distributions_perplexities.svg",
    )
    plot_hardness_distributions(
        variabilities=variabilities,
        output_path="figures/snli/snli_hardness_distributions_variabilities.svg",
    )
    plot_hardness_distributions(
        confidences=confidences,
        output_path="figures/snli/snli_hardness_distributions_confidences.svg",
    )

    # plot distributions of scaled hardness values
    if (
        perplexities is not None
        and variabilities is not None
        and confidences is not None
    ):
        min_p, max_p = min(perplexities).item(), max(perplexities).item()
        scaled_perplexities = [
            (val.item() - min_p) / (max_p - min_p) for val in perplexities
        ]
        min_v, max_v = min(variabilities).item(), max(variabilities).item()
        scaled_variabilities = [
            (val.item() - min_v) / (max_v - min_v) for val in variabilities
        ]
        min_c, max_c = min(confidences).item(), max(confidences).item()
        scaled_confidences = [
            (val.item() - min_c) / (max_c - min_c) for val in confidences
        ]
        print(
            f"lens: {len(scaled_perplexities)}, {len(scaled_variabilities)}, {len(scaled_confidences)}"
        )
        print(f"perplexities: {scaled_perplexities[:10]}")
        plot_hardness_distributions(
            perplexities=scaled_perplexities,
...
def plot_test_accuracies(filepaths, output_path):
    ...
    for filepath in filepaths:
        ...
        with open(filepath, "r") as f:
            _, _, test_acc, _, _, test_loss = (
                f.readlines()[-1].replace(": ", " ").replace(", ", " ").split()
            )
    ...
    ax = sns.barplot(
        x="Hardness Calculation",
        y="Test Accuracy",
        hue="Order",
        data=df[df["Hardness Calculation"] != "Baseline"],
    )
    # set y limits
    plt.ylim((0.6, 0.75))
    # Add a horizontal line for the baseline
    baseline_value = df[df["Hardness Calculation"] == "Baseline"]["Test Accuracy"].iloc[
        0
    ]
...
def main():
    ...
    plot_test_accuracies(
        multi_fpaths_test, "figures/snli/snli_scaled_test_accuracies.svg"
    )
    plot_test_accuracies(
        ordered_fpaths_test, "figures/snli/snli_ordered_test_accuracies.svg"
    )
